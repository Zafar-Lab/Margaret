...
class SeparableConvolution(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1,
                 padding=1, bias=False, dilation=1, norm='bn'):
        super(SeparableConvolution, self).__init__()
        self.conv1 = nn.Conv1d(
            in_channels, in_channels, kernel_size, stride, padding,
            groups=in_channels, bias=bias, dilation=dilation
        )
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=1,
                 base_width=64, dilation=1, grid_factor=1):
        super(Bottleneck, self).__init__()
        # Both self.conv2 and self.downsample layers downsample the input when stride != 1
        self.conv1 = nn.Conv1d(inplanes, planes, 1, bias=False)
        self.bn1 = nn.BatchNorm1d(planes)
        self.conv2 = nn.Conv1d(planes, planes, 3, stride=stride, padding=dilation * grid_factor, dilation=dilation * grid_factor, bias=False)
